...
export default function MyFileOrbis(props) 
{
  ...
  const handleBreadCrumbs = (e) => {
    ...
    navigate("/Team Folders/" + e.target.id);
  }

  const handleMainFolderNameClick = () => {
    setDirectoryPath("");
    navigate("/Team Folders");
  }
  ...
  const handleRowDoubleClick = () => {
    if(!isFile){
      ...
      navigate("/Team Folders/" + fullPath);
    } else {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const body = JSON.stringify({ "DType": 3, "DirectoryPath": directoryPath });
  ...
  return (
    <div style={{marginTop: "25px"}}>
      {/* constant "team folders" header path */}
      <Typography 
        id="team-folders" 
