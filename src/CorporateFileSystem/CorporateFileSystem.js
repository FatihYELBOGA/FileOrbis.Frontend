...
export default function MyFileOrbis(props) 
{
  ...
  const handleBreadCrumbs = (e) => {
    ...
    navigate("/Corporate File System/" + e.target.id);
  }

  const handleMainFolderNameClick = () => {
    setDirectoryPath("");
    navigate("/Corporate File System");
  }
  ...
  const handleRowDoubleClick = () => {
    if(!isFile){
      ...
      navigate("/Corporate File System/" + fullPath);
    } else {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const body = JSON.stringify({ "DType": 1, "DirectoryPath": directoryPath });
  ...
  return (
    <div style={{marginTop: "25px"}}>
      {/* constant "corporate file system" header path */}
      <Typography 
        id="my-fileorbis" 
