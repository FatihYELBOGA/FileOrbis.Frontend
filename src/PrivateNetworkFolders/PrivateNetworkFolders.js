...
export default function MyFileOrbis(props) 
{
  ...
  const handleBreadCrumbs = (e) => {
    ...
    navigate("/Private Network Folders/" + e.target.id);
  }

  const handleMainFolderNameClick = () => {
    setDirectoryPath("");
    navigate("/Private Network Folders");
  }
  ...
  const handleRowDoubleClick = () => {
    if(!isFile){
      ...
      navigate("/Private Network Folders/" + fullPath);
    } else {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const body = JSON.stringify({ "DType": 2, "DirectoryPath": directoryPath });
  ...
  return (
    <div style={{marginTop: "25px"}}>
      {/* constant "private network folders" header path */}
      <Typography 
        id="private-network-folders" 
        sx={{ fontSize: 15, display: "inline", cursor: 'pointer' }}
        onClick={() => {handleMainFolderNameClick();}}
      >
        Private Network Folders
      </Typography>
      {
        directoryPath != "" ? 
